...
#NUM_DEFAULT_TRAIN_EPOCHS = [60, 33, 25, 20, 15]
...
def process_command_args(arguments):
    ...
    if restore_epoch is None and level < 4:
        ...
        if restore_epoch == -1:
            print("Error: Cannot find any pre-trained models for level " + str(level + 1) + ".")
...
def process_test_model_args(arguments):
    ...
    if restore_epoch is None and orig_model == "false":
        ...
        if restore_iter == -1:
            print("Error: Cannot find any pre-trained models for level " + str(level) + ".")
            sys.exit()
...
def get_last_iter(level):
    ...
    saved_models = [int((model_file.split("_")[-1]).split(".")[0])
                    ...
                    if model_file.startswith("raw_to_tonemappedHDR_level_" + str(level))]
